...
// cur_pad, target
std::unordered_map<std::tuple<int, std::string>, uint64_t> cache{};

uint64_t find_min(const std::vector<Pad>& pads, const std::string& target, int cur_pad = 0) {
	if (cache.contains(std::tuple(cur_pad, target))) {
		return cache[std::tuple(cur_pad, target)];
	}
	...
	cache.emplace(std::tuple(cur_pad, target), cost);
	return cost;
}
...
int main(int argc, char** argv) {
	...
	runner.add_test_file("t2.txt", 2379451789590, 25);
	runner.add_test_file("t1.txt", 154115708116294, 25);
