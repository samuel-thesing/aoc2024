...
int solve(const std::string &input, const int min_cheat) {
	...
	for (const auto& pos : seen) {
		...
		for (int dx = -20; dx <= 20; ++dx) {
			for (int dy = -20 + std::abs(dx); dy <= 20 - std::abs(dx); ++dy) {
				auto new_pos = pos + Vec2i{dx, dy};
				auto it2 = dists.find(new_pos);
				if (it2 == dists.end()) continue;
				if (it2->second - it1->second - std::abs(dx) - std::abs(dy)  >= min_cheat) {
					result++;
				}
			}
		}
...
int main(int argc, char** argv) {
    ...
    runner.add_test_file("t1.txt", 3, 76);
    runner.add_test_file("t1.txt", 7, 74);
    runner.add_test_file("t1.txt", 29, 72);
